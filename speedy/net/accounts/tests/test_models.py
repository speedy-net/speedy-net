...
class NormalizeSlugTestCase(TestCase):
    def test_convert_to_lowercase(self):
        self.assertEqual(first=normalize_slug('CamelCase'), second='camelcase')

    def test_convert_dots_to_dashes(self):
        self.assertEqual(first=normalize_slug('one.dot'), second='one-dot')
        self.assertEqual(first=normalize_slug('two..dot.s'), second='two-dot-s')

    def test_convert_underscores_to_dashes(self):
        self.assertEqual(first=normalize_slug('one_underscore'), second='one-underscore')
        self.assertEqual(first=normalize_slug('two__under_scores'), second='two-under-scores')

    def test_convert_multiple_dashes_to_one(self):
        self.assertEqual(first=normalize_slug('three---dash---es'), second='three-dash-es')

    def test_cut_leading_symbols(self):
        self.assertEqual(first=normalize_slug('-dash'), second='dash')
        self.assertEqual(first=normalize_slug('..dots'), second='dots')
        self.assertEqual(first=normalize_slug('_under_score'), second='under-score')

    def test_cut_trailing_symbols(self):
        self.assertEqual(first=normalize_slug('dash-'), second='dash')
        self.assertEqual(first=normalize_slug('dots...'), second='dots')
        self.assertEqual(first=normalize_slug('under_score_'), second='under-score')


class NormalizeUsernameTestCase(TestCase):
    def test_this(self):
        self.assertEqual(first=normalize_username('this-is-a-slug'), second='thisisaslug')
        self.assertEqual(first=normalize_username('.this_is...a_slug--'), second='thisisaslug')
