...
urlpatterns = [
    ...
    url(r'^activate/done/$', generic.TemplateView.as_view(template_name='accounts/activate/done.html'), name='activate_done'),
    url(r'^activate/confirm/(?P<uidb64>[0-9A-Za-z_\-]+)/(?P<token>[0-9A-Za-z]{1,13}-[0-9A-Za-z]{1,20})/$', views.activate_confirm, name='activate_confirm'),
