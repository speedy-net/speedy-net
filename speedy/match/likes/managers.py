...
class UserLikeManager(BaseManager):
    ...
    def get_like_list_to_queryset(self, user):
        ...
        return self.filter(from_user=user).filter(to_user__in=liked_users).prefetch_related("to_user", "to_user__{}".format(SpeedyNetSiteProfile.RELATED_NAME), "to_user__{}".format(SpeedyMatchSiteProfile.RELATED_NAME), "to_user__email_addresses").distinct().order_by('-to_user__{}__last_visit'.format(SiteProfile.RELATED_NAME))
    ...
    def get_like_list_from_queryset(self, user):
        ...
        return self.filter(to_user=user).filter(from_user__in=who_likes_me).prefetch_related("from_user", "from_user__{}".format(SpeedyNetSiteProfile.RELATED_NAME), "from_user__{}".format(SpeedyMatchSiteProfile.RELATED_NAME), "from_user__email_addresses").distinct().order_by('-from_user__{}__last_visit'.format(SiteProfile.RELATED_NAME))
    ...
    def get_like_list_mutual_queryset(self, user):
        ...
        return self.filter(from_user=user, to_user_id__in=who_likes_me).prefetch_related("to_user", "to_user__{}".format(SpeedyNetSiteProfile.RELATED_NAME), "to_user__{}".format(SpeedyMatchSiteProfile.RELATED_NAME), "to_user__email_addresses").distinct().order_by('-to_user__{}__last_visit'.format(SiteProfile.RELATED_NAME))
