...
class SpeedyCoreAccountsLanguageMixin(SpeedyCoreBaseLanguageMixin):
    ...
    def _user_all_the_required_fields_keys(self):
        return [field_name.format(language_code=self.language_code) for field_name in ['first_name_{language_code}', 'username', 'slug', 'password', 'gender', 'date_of_birth']]
    ...
    def set_up(self):
        ...
        self.assertEqual(first=len(set(self._user_all_the_required_fields_keys())), second=6)
        self.assertEqual(first=len(set(self._cannot_create_user_without_all_the_required_fields_errors_dict_by_value(value=None).keys()) - set(self._user_all_the_required_fields_keys())), second=1)
        ...
        self.assertListEqual(list1=self._user_all_the_required_fields_keys()[:1], list2=[to_attribute(name='first_name')])
