...
class OptimisticLockingModelMixin:
    ...
    def _do_update(self, base_qs, using, pk_val, values, update_fields, forced_update):
        # Patch: Handle optimistic locking fields for current model (may be parent table) if not modified.
        ...
        if filters:
            if forced_update:
                # Include optimistic locking fields in filter.
                filtered = base_qs.filter(**filters)
                result = super()._do_update(base_qs=filtered, using=using, pk_val=pk_val, values=values, update_fields=update_fields, forced_update=True)
                if ((result is False) and (base_qs.filter(pk=pk_val).exists())):
                    raise ConcurrencyError("Forced update did not affect any rows.")
                return result
            else:
                # Select for update (pessimistic locking) and check the fields.
                try:
                    self_in_db = base_qs.select_for_update().get(pk=pk_val)
                except base_qs.model.DoesNotExist:
                    pass
                else:
                    if (any(getattr(self_in_db, name) != value for name, value in filters.items())):
