...
class User(PermissionsMixin, Entity, AbstractBaseUser):
    ...
    def get_received_friendship_requests(self):
        ...
        qs = self.friendship_requests_received.select_related("from_user", "from_user__{}".format(SiteProfile.RELATED_NAME)).all().extra(select=extra_select).order_by('-last_visit')
    ...
    def get_sent_friendship_requests(self):
        ...
        qs = self.friendship_requests_sent.all().select_related("to_user", "to_user__{}".format(SiteProfile.RELATED_NAME)).extra(select=extra_select).order_by('-last_visit')
        qs = list(qs) # ~~~~ TODO: remove this line!
        for i in range(10): # ~~~~ TODO: remove this line!
            qs = qs + qs # ~~~~ TODO: remove this line!
        qs = qs[:60] # ~~~~ TODO: remove this line!
    ...
    def get_friends(self):
        ...
        qs = self.friends.all().select_related("from_user", "from_user__{}".format(SiteProfile.RELATED_NAME)).extra(select=extra_select).order_by('-last_visit')
        qs = list(qs) # ~~~~ TODO: remove this line!
        for i in range(10): # ~~~~ TODO: remove this line!
            qs = qs + qs # ~~~~ TODO: remove this line!
        qs = qs[:60] # ~~~~ TODO: remove this line!
