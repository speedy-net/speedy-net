...
class User(PermissionsMixin, Entity, AbstractBaseUser):
    ...
    def get_received_friendship_requests(self):
        ...
        qs = self.friendship_requests_received.all().extra(select=extra_select).prefetch_related("from_user", "from_user__{}".format(SiteProfile.RELATED_NAME)).distinct().order_by('-last_visit')
    ...
    def get_sent_friendship_requests(self):
        ...
        qs = self.friendship_requests_sent.all().extra(select=extra_select).prefetch_related("to_user", "to_user__{}".format(SiteProfile.RELATED_NAME)).distinct().order_by('-last_visit')
    ...
    def get_friends(self):
        ...
        qs = self.friends.all().extra(select=extra_select).prefetch_related("from_user", "from_user__{}".format(SiteProfile.RELATED_NAME)).distinct().order_by('-last_visit')
