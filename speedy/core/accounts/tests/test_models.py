    ...
    if (django_settings.LOGIN_ENABLED):
        class UserTestCaseMixin(SpeedyCoreAccountsModelsMixin, SpeedyCoreAccountsLanguageMixin):
            ...
            def test_user_profile_last_visit_str(self):
                ...
                self.assertIs(expr1={'en': "(2\u00A0days ago)", 'he': "(לפני יומיים)"}[self.language_code] in user_3.profile.last_visit_str, expr2=True)
                user_4 = ActiveUserFactory()
                user_4.profile.last_visit -= relativedelta(days=3)
                user_4.save_user_and_profile()
                self.assertIs(expr1={'en': "(3\u00A0days ago)", 'he': "(לפני 3\u00A0ימים)"}[self.language_code] in user_4.profile.last_visit_str, expr2=True)
                user_5 = ActiveUserFactory()
                user_5.profile.last_visit -= relativedelta(weeks=1)
                user_5.save_user_and_profile()
                self.assertIs(expr1={'en': "(1\u00A0week ago)", 'he': "(לפני שבוע)"}[self.language_code] in user_5.profile.last_visit_str, expr2=True)
                user_6 = ActiveUserFactory()
                user_6.profile.last_visit -= relativedelta(weeks=2)
                user_6.save_user_and_profile()
                self.assertIs(expr1={'en': "(2\u00A0weeks ago)", 'he': "(לפני שבועיים)"}[self.language_code] in user_6.profile.last_visit_str, expr2=True)
                user_7 = ActiveUserFactory()
                user_7.profile.last_visit -= relativedelta(months=1)
                user_7.save_user_and_profile()
                self.assertIs(expr1={'en': "(1\u00A0month ago)", 'he': "(לפני חודש)"}[self.language_code] in user_7.profile.last_visit_str, expr2=True)
                user_8 = ActiveUserFactory()
                user_8.profile.last_visit -= relativedelta(months=2)
                user_8.save_user_and_profile()
                self.assertIs(expr1={'en': "(2\u00A0months ago)", 'he': "(לפני חודשיים)"}[self.language_code] in user_8.profile.last_visit_str, expr2=True)
                user_9 = ActiveUserFactory()
                user_9.profile.last_visit -= relativedelta(months=3)
                user_9.save_user_and_profile()
                self.assertIs(expr1={'en': "(3\u00A0months ago)", 'he': "(לפני 3\u00A0חודשים)"}[self.language_code] in user_9.profile.last_visit_str, expr2=True)
                user_10 = ActiveUserFactory()
                user_10.profile.last_visit -= relativedelta(months=3, weeks=1)
                user_10.save_user_and_profile()
                self.assertIs(expr1={'en': "(3\u00A0months, 1\u00A0week ago)", 'he': "(לפני 3\u00A0חודשים, שבוע)"}[self.language_code] in user_10.profile.last_visit_str, expr2=True)
                user_11 = ActiveUserFactory()
                user_11.profile.last_visit -= relativedelta(months=3, weeks=2)
                user_11.save_user_and_profile()
                self.assertIs(expr1={'en': "(3\u00A0months, 2\u00A0weeks ago)", 'he': "(לפני 3\u00A0חודשים, שבועיים)"}[self.language_code] in user_11.profile.last_visit_str, expr2=True)
                user_12 = ActiveUserFactory()
                user_12.profile.last_visit -= relativedelta(years=1)
                user_12.save_user_and_profile()
                self.assertIs(expr1={'en': "(1\u00A0year ago)", 'he': "(לפני שנה)"}[self.language_code] in user_12.profile.last_visit_str, expr2=True)
                user_13 = ActiveUserFactory()
                user_13.profile.last_visit -= relativedelta(years=1, weeks=1)
                user_13.save_user_and_profile()
                self.assertIs(expr1={'en': "(1\u00A0year ago)", 'he': "(לפני שנה)"}[self.language_code] in user_13.profile.last_visit_str, expr2=True)
                user_14 = ActiveUserFactory()
                user_14.profile.last_visit -= relativedelta(years=1, months=1)
                user_14.save_user_and_profile()
                self.assertIs(expr1={'en': "(1\u00A0year, 1\u00A0month ago)", 'he': "(לפני שנה, חודש)"}[self.language_code] in user_14.profile.last_visit_str, expr2=True)
                user_15 = ActiveUserFactory()
                user_15.profile.last_visit -= relativedelta(years=1, months=1, weeks=1)
                user_15.save_user_and_profile()
                self.assertIs(expr1={'en': "(1\u00A0year, 1\u00A0month ago)", 'he': "(לפני שנה, חודש)"}[self.language_code] in user_15.profile.last_visit_str, expr2=True)
                user_16 = ActiveUserFactory()
                user_16.profile.last_visit -= relativedelta(years=2)
                user_16.save_user_and_profile()
                self.assertIs(expr1={'en': "(2\u00A0years ago)", 'he': "(לפני שנתיים)"}[self.language_code] in user_16.profile.last_visit_str, expr2=True)
                user_17 = ActiveUserFactory()
                user_17.profile.last_visit -= relativedelta(years=2, weeks=2)
                user_17.save_user_and_profile()
                self.assertIs(expr1={'en': "(2\u00A0years ago)", 'he': "(לפני שנתיים)"}[self.language_code] in user_17.profile.last_visit_str, expr2=True)
                ...
                self.assertIs(expr1={'en': "(1\u00A0year, 11\u00A0months ago)", 'he': "(לפני שנה, 11\u00A0חודשים)"}[self.language_code] in user_20.profile.last_visit_str, expr2=True)
