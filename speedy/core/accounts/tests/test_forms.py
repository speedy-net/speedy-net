...
from speedy.core.base.test import ErrorsMixin, TestCase, only_on_sites_with_login, exclude_on_speedy_match
...
class PasswordResetFormTestCase(TestCase):
    def setup(self):
        ...
        self.unconfirmed_email = UserEmailAddressFactory(user=self.user, is_confirmed=False)
        self.other_user_email = UserEmailAddressFactory(user=self.other_user, is_confirmed=True)
        self.form = PasswordResetForm(language_code=self.language_code)
...
class DeactivationFormTestCase(ErrorsMixin, TestCase):
    ...
    def test_incorrect_password(self):
        ...
        form = SiteProfileDeactivationForm(language_code=self.language_code, user=self.user, data=data)
        self.assertFalse(expr=form.is_valid())
