...
class AcceptFriendRequestViewTestCase(TestCase):
    def setUp(self):
        self.user = UserFactory()
        self.other_user = UserFactory()
        self.frequest = Friend.objects.add_friend(self.user, self.other_user)
        self.page_url = '/{}/friends/request/accept/{}/'.format(self.other_user.slug, self.frequest.id)
    ...
    def test_user_that_has_received_request_can_accept_it(self):
        ...
        self.assertEqual(first=FriendshipRequest.objects.count(), second=0)
...
class RejectFriendRequestViewTestCase(TestCase):
    def setUp(self):
        ...
        self.other_user = UserFactory()
        self.frequest = Friend.objects.add_friend(self.user, self.other_user)
        self.page_url = '/{}/friends/request/reject/{}/'.format(self.other_user.slug, self.frequest.id)
