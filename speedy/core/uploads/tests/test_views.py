    ...
    if (django_settings.LOGIN_ENABLED):
        ...
        class UploadViewOnlyEnglishTestCase(SiteTestCase):
            ...
            def test_upload_file(self):
                ...
                self.assertEqual(first=len(stem_sha384), second=96)
                self.assertEqual(first=len(json_response['files'][0]['name']), second=32)
                ...
                self.assertEqual(first=len(str(image.id)), second=20)
                self.assertEqual(first=len(image.basename), second=32)
